...
class StyleModel extends TaxonomyModel
{
    ...
    public function validateAddNewStyle($para)
    {
        ...
        if (isset($para->name) && $para->name != "") {
            if ($this->hasTaxonomyWithName($para->name, "style")) {
        ...
        if (isset($para->slug) && $para->slug != "") {
            if ($this->hasTaxonomyWithSlug($para->slug, "style")) {
    ...
    public function addNewStyle($para)
    {
        try {
            if ($this->validateAddNewStyle($para)) {
                BO::autoloadBO("style");
                $styleBO = new StyleBO();
                ...
                if ($this->addTaxonomyToDatabase($styleBO)) {
                    $this->db->commit();
    ...
    public function validateUpdateInfoStyle($para)
    {
        if ($para == null || !is_object($para)) {
    ...
    public function updateInfoStyle($para)
    {
        try {
            if ($this->validateUpdateInfoStyle($para)) {
                $styleBO = $this->getTerm($para->term_taxonomy_id);
                if ($styleBO != NULL) {
                    if (isset($para->name)) {
                        $styleBO->name = $para->name;
                    }
    ...
    public function updateEditStylePerPages($styles_per_page)
    {
    ...
    public function changeAdvSetting($para)
    {
        ...
        if (isset($para->styles_per_page) && is_numeric($para->styles_per_page)) {
            $this->updateEditStylePerPages($para->styles_per_page);
        }
    ...
    public function searchStyle($view, $para)
    {
        ...
        $taxonomy = "style";
        parent::searchTaxonomy($view, $para, $styles_per_page, $taxonomy);
    }
}
