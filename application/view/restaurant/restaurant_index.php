...
if (isset($userBO->manage_restaurants_columns_show)) {
    if (isset($userBO->manage_restaurants_columns_show->city_name_show)) {
        $city_name_show = $userBO->manage_restaurants_columns_show->city_name_show;
    } else {
        $city_name_show = true;
    }
    if (isset($userBO->manage_restaurants_columns_show->country_name_show)) {
        $country_name_show = $userBO->manage_restaurants_columns_show->country_name_show;
    } else {
        $country_name_show = true;
    }
    if (isset($userBO->manage_restaurants_columns_show->current_rating_show)) {
        $current_rating_show = $userBO->manage_restaurants_columns_show->current_rating_show;
    } else {
        $current_rating_show = true;
    }
    if (isset($userBO->manage_restaurants_columns_show->vote_times_show)) {
        $vote_times_show = $userBO->manage_restaurants_columns_show->vote_times_show;
    } else {
...
if (isset($userBO->restaurants_per_page)) {
    $restaurants_per_page = $userBO->restaurants_per_page;
} else if (isset($_SESSION['options']) && isset($_SESSION['options']->restaurants_per_page)) {
    $restaurants_per_page = $_SESSION['options']->restaurants_per_page;
} else {
...
if (!(isset($this->ajax) && $this->ajax)) {
            ...
            <form method="post" id="adv-settings" >
                <fieldset class="metabox-prefs">
                    ...
                    <label><input type="checkbox" <?php if (isset($city_name_show) && $city_name_show) { ?> 
                                      checked="checked" <?php } ?>
                                  value="city" name="city_name_show" class="hide-column-tog"><?php echo RESTAURANT_CITY_TITLE; ?></label>
                                  <label><input type="checkbox" <?php if (isset($country_name_show) && $country_name_show) { ?> 
                                      checked="checked" <?php } ?>
                                  value="country" name="country_name_show" class="hide-column-tog"><?php echo RESTAURANT_CITY_TITLE; ?></label>
                ...
                <fieldset class="screen-options">
                    ...
                    <label for="restaurants_per_page"><?php echo NUMBER_OF_ITEMS_PER_PAGE_DESC; ?></label>
                    <input type="number" value="<?php
                    if (isset($restaurants_per_page)) {
                        echo $restaurants_per_page;
                    } else {
                        echo RESTAURANTS_PER_PAGE_DEFAULT;
                    }

                    ?>" maxlength="3" id="restaurants_per_page" name="restaurants_per_page" class="screen-per-page" max="999" min="1" step="1">
                </fieldset>
    ...
    <h1>
        <?php echo DASHBOARD_RESTAURANT_TITLE; ?> 
        <a class="page-title-action" onclick="getAddNewPage(this)" ><?php echo ADD_NEW_TITLE; ?></a>
    </h1>

    <?php $this->renderFeedbackMessages(); ?>

    <form id="form-restaurant-edit" method="post" onsubmit="submitFormRestaurantEdit(event)">
        ...
        <p class="search-box">
            <label for="restaurant-search-input" class="screen-reader-text">
                <?php echo SEARCH_RESTAURANTS_TITLE; ?>:</label>
            <input type="search" value="<?php
            if (isset($this->s)) {
                echo htmlspecialchars($this->s);
            }

            ?>" name="s" id="restaurant-search-input" />
            <input type="submit" value="<?php echo SEARCH_RESTAURANTS_TITLE; ?>" class="button" id="search-submit" />
        </p>
        ...
        <h2 class="screen-reader-text"><?php echo RESTAURANTS_LIST_TITLE; ?></h2>
        <table class="wp-list-table widefat fixed striped restaurants">
            <thead>
                <tr>
                    ...
                    if (isset($this->orderby) && $this->orderby == "post_title" && in_array($this->order, array('asc', 'desc'))) {
                        ...
                        <th class="manage-column column-post_title sorted <?php echo $this->order; ?>" id="post_title" scope="col">
                            <a href="#" orderby="post_title" order="<?php echo $this->order; ?>" onclick="filterOrderBy(this)">
                                <span><?php echo RESTAURANT_NAME_TITLE; ?></span>
                    ...
                    if (isset($this->orderby) && $this->orderby == "city_name" && in_array($this->order, array('asc', 'desc'))) {
                        ...
                        ?> sorted <?php echo $this->order; ?>" id="city_name" scope="col">
                            <a href="#" orderby="city_name" order="<?php echo $this->order; ?>" onclick="filterOrderBy(this)">
                                <span><?php echo RESTAURANT_CITY_TITLE; ?></span>
                                <span class="sorting-indicator"></span>
                            </a>
                        </th>   
                        <?php
            ...
            <tbody data-wp-lists="list:restaurant" id="the-list">
                <?php
                if (!is_null($this->restaurantList)) {
                    foreach ($this->restaurantList as $restaurantInfo) {

                        ?>
                        <tr id="restaurant-<?php echo $restaurantInfo->ID; ?>">
                            <th class="check-column" scope="row">
                                <label for="restaurant_<?php echo $restaurantInfo->ID; ?>" class="screen-reader-text"><?php echo SELECT_TITLE; ?> <?php echo htmlspecialchars($restaurantInfo->post_title); ?></label>
                                <input type="checkbox" value="<?php echo $restaurantInfo->ID; ?>" class="author" id="restaurant_<?php echo $restaurantInfo->ID; ?>" name="restaurants[]" >
                            </th>

                            <td data-colname="<?php echo RESTAURANT_NAME_TITLE; ?>" class="username column-username has-row-actions column-primary">
                                <img width="32" height="32" class="image image-32 photo" srcset="<?php
                                if (isset($restaurantInfo->image_url)) {
                                    echo URL . htmlspecialchars($restaurantInfo->image_url);
                                } else {
                                ...
                                ?>" src="<?php
                                     ...
                                     ?>" alt=""> 
                                <strong>
                                    <a href="#" restaurant="<?php echo $restaurantInfo->ID; ?>" name="<?php echo htmlspecialchars($restaurantInfo->post_name); ?>" onclick="getRestaurantInfoPage(this)"><?php echo htmlspecialchars($restaurantInfo->post_title); ?></a>
                                </strong>
                                <br>
                                <div class="row-actions">
                                    <span class="view">
                                        <a href="#" restaurant="<?php echo $restaurantInfo->ID; ?>" name="<?php echo htmlspecialchars($restaurantInfo->post_name); ?>" onclick="viewRestaurant(this)"><?php echo VIEW_TITLE; ?>
                                        </a>
                                    </span> |
                                    <span class="edit">
                                        <a href="#" restaurant="<?php echo $restaurantInfo->ID; ?>" name="<?php echo htmlspecialchars($restaurantInfo->post_name); ?>" onclick="getEditRestaurantPage(this)"><?php echo EDIT_TITLE; ?>
                                        </a>
                                    </span> |
                                    <span class="delete">
                                        <a href="#" class="submitdelete" restaurant="<?php echo $restaurantInfo->ID; ?>" name="<?php echo htmlspecialchars($restaurantInfo->post_title); ?>" onclick="deleteRestaurant(this)"><?php echo DELETE_TITLE; ?>
                                        </a>
                                    </span>
                                </div>
                            ...
                            <td data-colname="<?php echo RESTAURANT_CITY_TITLE; ?>" class="slug column-city <?php
                            if (!(isset($city_name_show) && $city_name_show)) {
                                echo " hidden ";
                            }

                            ?>"><?php echo htmlspecialchars($restaurantInfo->city_name); ?></td>
                            ...
                            ?>"><?php echo htmlspecialchars($restaurantInfo->country_name); ?></td>
                            ...
                            ?>"><?php echo htmlspecialchars($restaurantInfo->current_rating); ?></td>
                            ...
                            ?>"><?php echo htmlspecialchars($restaurantInfo->vote_times); ?></td>
                        </tr>      
                        <?php
                    }
                }
...
if (!(isset($this->ajax) && $this->ajax)) {
    ...
    <script>
        jQuery("#adv-settings").submit(function (e) {
            ...
            var postDataSearch = jQuery("#form-restaurant-edit").serializeArray();
            ...
            searchRestaurant(postData);
        });

        //        jQuery("#form-restaurant-edit").submit(function (e) {
        //            e.preventDefault(); //STOP default action
        //            var postData = jQuery(this).serializeArray();
        //            searchRestaurant(postData);
        //        });

        function submitFormRestaurantEdit(e) {
            e.preventDefault(); //STOP default action
            try {
                var postData = jQuery("#form-restaurant-edit").serializeArray();
                searchRestaurant(postData);
            } catch (e) {
        ...
        function searchRestaurant(postData) {
            jQuery.ajax({
                url: "",
                type: "POST",
                data: postData,
        ...
        function filterOrderBy(element) {
            ...
            jQuery('#form-restaurant-edit input[name="orderby"]').val(orderby);
            jQuery('#form-restaurant-edit input[name="order"]').val(order);
            var postData = jQuery("#form-restaurant-edit").serializeArray();
        ...
        function filterPage(element) {
            ...
            jQuery('#form-restaurant-edit input[name="page"]').val(page);
            var postData = jQuery("#form-restaurant-edit").serializeArray();
            //        var formURL = jQuery(this).attr("action");
            searchRestaurant(postData);
        }
        ...
        function getEditRestaurantPage(element) {
            var ID = jQuery(element).attr("restaurant");
            var name = jQuery(element).attr("name");
            var url = "<?php echo URL . CONTEXT_PATH_RESTAURANT_EDIT_INFO; ?>" + ID + "/" + name;
        ...
        function getAddNewPage(element) {
            var url = "<?php echo URL . CONTEXT_PATH_RESTAURANT_ADD_NEW; ?>";
            if (url != null && url != "" && url != undefined) {
                var win = window.open(url, '_blank');
                win.focus();
            }
        ...
        function getRestaurantInfoPage(element) {
            var restaurant = jQuery(element).attr("restaurant");
            var name = jQuery(element).attr("name");
            var url = "<?php echo URL . CONTEXT_PATH_RESTAURANT_INFO; ?>" + restaurant + "/" + name;
            if (url != null && url != "" && url != undefined) {
                var win = window.open(url, '_blank');
                win.focus();
            }
        }
        function viewRestaurant(element) {
            var restaurant = jQuery(element).attr("restaurant");
            var name = jQuery(element).attr("name");
            var url = "<?php echo URL . CONTEXT_PATH_RESTAURANT_VIEW; ?>" + restaurant + "/" + name;
            if (url != null && url != "" && url != undefined) {
                var win = window.open(url, '_blank');
                win.focus();
            }
        ...
        function deleteRestaurant(element) {
            var restaurant = jQuery(element).attr("restaurant");
            var name = jQuery(element).attr("name");
            if (confirm('<?php echo CONFIRM_DELETE_RESTAURANT; ?>' + name + '<?php echo CONFIRM_DELETE_CANCEL_OK; ?>')) {
                jQuery("#cb-select-all-1").prop('checked', false);
                jQuery("#cb-select-all-2").prop('checked', false);
                jQuery("input[name='restaurants[]'][type=checkbox]").prop('checked', false);
                jQuery("input[name='restaurants[]'][type=checkbox][value='" + restaurant + "']").prop('checked', true);

                jQuery('#form-restaurant-edit select[name="action"] option').removeAttr('selected');
                jQuery('#form-restaurant-edit select[name="action2"] option').removeAttr('selected');
                jQuery('#form-restaurant-edit select[name="action"] option[value="delete"]').attr('selected', true);
                jQuery('#form-restaurant-edit input[name="type"]').val('action');
                var postData = jQuery("#form-restaurant-edit").serializeArray();
                //        var formURL = jQuery(this).attr("action");
                searchRestaurant(postData);
        ...
        function applyAction(type) {
            if (confirm('<?php echo CONFIRM_ACTION_DESTINATION . CONFIRM_ACTION_CANCEL_OK; ?>')) {
                jQuery('#form-restaurant-edit input[name="type"]').val(type);
                var postData = jQuery("#form-restaurant-edit").serializeArray();
                //        var formURL = jQuery(this).attr("action");
                searchRestaurant(postData);
            }
        ...
        function checkAll(element) {
            if (jQuery(element).prop('checked')) {
                ...
                jQuery("input[name='restaurants[]'][type=checkbox]").prop('checked', true);
            } else {
                jQuery("#cb-select-all-1").prop('checked', false);
                jQuery("#cb-select-all-2").prop('checked', false);
                jQuery("input[name='restaurants[]'][type=checkbox]").prop('checked', false);
