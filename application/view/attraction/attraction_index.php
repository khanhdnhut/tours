...
if (isset($userBO->manage_attractions_columns_show)) {
    if (isset($userBO->manage_attractions_columns_show->description_show)) {
        $description_show = $userBO->manage_attractions_columns_show->description_show;
    } else {
        $description_show = true;
    }
    if (isset($userBO->manage_attractions_columns_show->slug_show)) {
        $slug_show = $userBO->manage_attractions_columns_show->slug_show;
    } else {
        $slug_show = true;
    }
    if (isset($userBO->manage_attractions_columns_show->tours_show)) {
        $tours_show = $userBO->manage_attractions_columns_show->tours_show;
    } else {
        $tours_show = true;
    }
} else {
...
if (isset($userBO->attractions_per_page)) {
    $attractions_per_page = $userBO->attractions_per_page;
} else if (isset($_SESSION['options']) && isset($_SESSION['options']->attractions_per_page)) {
    $attractions_per_page = $_SESSION['options']->attractions_per_page;
} else {
...
if (!(isset($this->ajax) && $this->ajax)) {
        ...
        <div aria-label="Screen Options Tab" tabindex="-1" class="hidden" id="screen-options-wrap">
            <form method="post" id="adv-settings" >
                ...
                <fieldset class="screen-options">
                    ...
                    <label for="attractions_per_page"><?php echo NUMBER_OF_ITEMS_PER_PAGE_DESC; ?></label>
                    <input type="number" value="<?php
                    if (isset($attractions_per_page)) {
                        echo $attractions_per_page;
                    } else {
                        echo ATTRACTIONS_PER_PAGE_DEFAULT;
                    }

                    ?>" maxlength="3" id="attractions_per_page" name="attractions_per_page" class="screen-per-page" max="999" min="1" step="1">
                </fieldset>
    ...
    <h1>
        <?php echo DASHBOARD_ATTRACTION_TITLE; ?> 
        <a class="page-title-action" onclick="getAddNewPage(this)" ><?php echo ADD_NEW_TITLE; ?></a>
    </h1>

    <?php $this->renderFeedbackMessages(); ?>

    <form id="form-attraction-edit" method="post" onsubmit="submitFormAttractionEdit(event)">
        ...
        <p class="search-box">
            <label for="attraction-search-input" class="screen-reader-text">
                <?php echo SEARCH_ATTRACTIONS_TITLE; ?>:</label>
            <input type="search" value="<?php
            if (isset($this->s)) {
                echo htmlspecialchars($this->s);
            }

            ?>" name="s" id="attraction-search-input" />
            <input type="submit" value="<?php echo SEARCH_ATTRACTIONS_TITLE; ?>" class="button" id="search-submit" />
        </p>
        ...
        <h2 class="screen-reader-text"><?php echo ATTRACTIONS_LIST_TITLE; ?></h2>
        <table class="wp-list-table widefat fixed striped attractions">
            <thead>
                <tr>
                    ...
                    if (isset($this->orderby) && $this->orderby == "name" && in_array($this->order, array('asc', 'desc'))) {
                        ...
                        ?>
                        <th title="Name of attraction" class="manage-column column-name sorted <?php echo $this->order; ?>" id="name" scope="col">
                        ...
                        ?>
                        <th title="Name of attraction" class="manage-column column-name sortable desc" id="name" scope="col">
                    ...
                    if (isset($this->orderby) && $this->orderby == "description" && in_array($this->order, array('asc', 'desc'))) {
                        ...
                        ?>
                        <th title="Description of attraction" class="manage-column column-description <?php
                    ...
                    if (isset($this->orderby) && $this->orderby == "slug" && in_array($this->order, array('asc', 'desc'))) {
                        ...
                        ?>
                        <th title="Slug of attraction" class="manage-column column-slug <?php
                    ...
                    if (isset($this->orderby) && $this->orderby == "tours" && in_array($this->order, array('asc', 'desc'))) {
                        ...
                        ?>
                        <th title="Count tours of attraction" class="manage-column column-tours <?php
            ...
            <tbody data-wp-lists="list:attraction" id="the-list">
                <?php
                if (!is_null($this->taxonomyList)) {
                    foreach ($this->taxonomyList as $taxonomyInfo) {

                        ?>
                        <tr id="attraction-<?php echo $taxonomyInfo->term_taxonomy_id; ?>">
                            <th class="check-column" scope="row">
                                <label for="attraction_<?php echo $taxonomyInfo->term_taxonomy_id; ?>" class="screen-reader-text"><?php echo SELECT_TITLE; ?> <?php echo htmlspecialchars($taxonomyInfo->name); ?></label>
                                <input type="checkbox" value="<?php echo $taxonomyInfo->term_taxonomy_id; ?>" class="author" id="attraction_<?php echo $taxonomyInfo->term_taxonomy_id; ?>" name="attractions[]" >
                            </th>
                            <td data-colname="name" class="name column-name has-row-actions column-primary">                                
                                <strong>
                                    <a href="#" attraction="<?php echo $taxonomyInfo->term_taxonomy_id; ?>" name="<?php echo htmlspecialchars($taxonomyInfo->name); ?>" onclick="getAttractionInfoPage(this)"><?php echo htmlspecialchars($taxonomyInfo->name); ?></a>
                                </strong>
                                <br>
                                <div class="row-actions">
                                    <span class="view">
                                        <a href="#" attraction="<?php echo $taxonomyInfo->term_taxonomy_id; ?>" name="<?php echo htmlspecialchars($taxonomyInfo->slug); ?>" onclick="viewAttraction(this)"><?php echo VIEW_TITLE; ?>
                                        </a>
                                    </span> |                                    
                                    <span class="edit">
                                        <a href="#" attraction="<?php echo $taxonomyInfo->term_taxonomy_id; ?>" name="<?php echo htmlspecialchars($taxonomyInfo->slug); ?>" onclick="getEditAttractionPage(this)"><?php echo EDIT_TITLE; ?>
                                        </a>
                                    </span>
                                    | <span class="delete">
                                        <a href="#" class="submitdelete" attraction="<?php echo $taxonomyInfo->term_taxonomy_id; ?>" name="<?php echo htmlspecialchars($taxonomyInfo->name); ?>" onclick="deleteAttraction(this)"><?php echo DELETE_TITLE; ?>
                                        </a>
                                    </span>
                                </div>
...
if (!(isset($this->ajax) && $this->ajax)) {
    ...
    <script>
        jQuery("#adv-settings").submit(function (e) {
            ...
            var postDataSearch = jQuery("#form-attraction-edit").serializeArray();
            ...
            searchAttraction(postData);
        });

        //        jQuery("#form-attraction-edit").submit(function (e) {
        //            e.preventDefault(); //STOP default action
        //            var postData = jQuery(this).serializeArray();
        //            searchAttraction(postData);
        //        });

        function submitFormAttractionEdit(e) {
            e.preventDefault(); //STOP default action
            try {
                var postData = jQuery("#form-attraction-edit").serializeArray();
                searchAttraction(postData);
            } catch (e) {
        ...
        function searchAttraction(postData) {
            jQuery.ajax({
                url: "",
                type: "POST",
                data: postData,
        ...
        function filterOrderBy(element) {
            ...
            jQuery('#form-attraction-edit input[name="orderby"]').val(orderby);
            jQuery('#form-attraction-edit input[name="order"]').val(order);
            var postData = jQuery("#form-attraction-edit").serializeArray();
        ...
        function filterPage(element) {
            ...
            jQuery('#form-attraction-edit input[name="page"]').val(page);
            var postData = jQuery("#form-attraction-edit").serializeArray();
            //        var formURL = jQuery(this).attr("action");
            searchAttraction(postData);
        }
        ...
        function getEditAttractionPage(element) {
            var term_taxonomy_id = jQuery(element).attr("attraction");
            var name = jQuery(element).attr("name");
            var url = "<?php echo URL . CONTEXT_PATH_ATTRACTION_EDIT_INFO; ?>" + term_taxonomy_id + "/" + name;
            if (url != null && url != "" && url != undefined) {
                var win = window.open(url, '_blank');
                win.focus();
            }
        }
        function viewAttraction(element) {
            var term_taxonomy_id = jQuery(element).attr("attraction");
            var name = jQuery(element).attr("name");
            var url = "<?php echo URL . CONTEXT_PATH_ATTRACTION_VIEW; ?>" + term_taxonomy_id + "/" + name;
            if (url != null && url != "" && url != undefined) {
                var win = window.open(url, '_blank');
                win.focus();
            }
        ...
        function getAddNewPage(element) {
            var url = "<?php echo URL . CONTEXT_PATH_ATTRACTION_ADD_NEW; ?>";
            if (url != null && url != "" && url != undefined) {
                var win = window.open(url, '_blank');
                win.focus();
            }
        ...
        function getAttractionInfoPage(element) {
            var attraction = jQuery(element).attr("attraction");
            var name = jQuery(element).attr("name");
            var url = "<?php echo URL . CONTEXT_PATH_ATTRACTION_INFO; ?>" + attraction + "/" + name;
            if (url != null && url != "" && url != undefined) {
                var win = window.open(url, '_blank');
                win.focus();
            }
        ...
        function deleteAttraction(element) {
            var attraction = jQuery(element).attr("attraction");
            var name = jQuery(element).attr("name");
            if (confirm('<?php echo CONFIRM_DELETE_ATTRACTION; ?>' + name + '<?php echo CONFIRM_DELETE_CANCEL_OK; ?>')) {
                jQuery("#cb-select-all-1").prop('checked', false);
                jQuery("#cb-select-all-2").prop('checked', false);
                jQuery("input[name='attractions[]'][type=checkbox]").prop('checked', false);
                jQuery("input[name='attractions[]'][type=checkbox][value='" + attraction + "']").prop('checked', true);

                jQuery('#form-attraction-edit select[name="action"] option').removeAttr('selected');
                jQuery('#form-attraction-edit select[name="action2"] option').removeAttr('selected');
                jQuery('#form-attraction-edit select[name="action"] option[value="delete"]').attr('selected', true);
                jQuery('#form-attraction-edit input[name="type"]').val('action');
                var postData = jQuery("#form-attraction-edit").serializeArray();
                //        var formURL = jQuery(this).attr("action");
                searchAttraction(postData);
        ...
        function applyAction(type) {
            if (confirm('<?php echo CONFIRM_ACTION_ATTRACTION . CONFIRM_ACTION_CANCEL_OK; ?>')) {
                jQuery('#form-attraction-edit input[name="type"]').val(type);
                var postData = jQuery("#form-attraction-edit").serializeArray();
                //        var formURL = jQuery(this).attr("action");
                searchAttraction(postData);
            }
        ...
        function checkAll(element) {
            if (jQuery(element).prop('checked')) {
                ...
                jQuery("input[name='attractions[]'][type=checkbox]").prop('checked', true);
            } else {
                jQuery("#cb-select-all-1").prop('checked', false);
                jQuery("#cb-select-all-2").prop('checked', false);
                jQuery("input[name='attractions[]'][type=checkbox]").prop('checked', false);
