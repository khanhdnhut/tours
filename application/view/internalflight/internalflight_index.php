...
if (isset($userBO->manage_internalflights_columns_show)) {
    if (isset($userBO->manage_internalflights_columns_show->city_name_show)) {
        $city_name_show = $userBO->manage_internalflights_columns_show->city_name_show;
    } else {
        $city_name_show = true;
    }
    if (isset($userBO->manage_internalflights_columns_show->country_name_show)) {
        $country_name_show = $userBO->manage_internalflights_columns_show->country_name_show;
    } else {
        $country_name_show = true;
    }
    if (isset($userBO->manage_internalflights_columns_show->current_rating_show)) {
        $current_rating_show = $userBO->manage_internalflights_columns_show->current_rating_show;
    } else {
        $current_rating_show = true;
    }
    if (isset($userBO->manage_internalflights_columns_show->vote_times_show)) {
        $vote_times_show = $userBO->manage_internalflights_columns_show->vote_times_show;
    } else {
...
if (isset($userBO->internalflights_per_page)) {
    $internalflights_per_page = $userBO->internalflights_per_page;
} else if (isset($_SESSION['options']) && isset($_SESSION['options']->internalflights_per_page)) {
    $internalflights_per_page = $_SESSION['options']->internalflights_per_page;
} else {
...
if (!(isset($this->ajax) && $this->ajax)) {
            ...
            <form method="post" id="adv-settings" >
                <fieldset class="metabox-prefs">
                    ...
                    <label><input type="checkbox" <?php if (isset($city_name_show) && $city_name_show) { ?> 
                                      checked="checked" <?php } ?>
                                  value="city" name="city_name_show" class="hide-column-tog"><?php echo INTERNALFLIGHT_CITY_TITLE; ?></label>
                                  <label><input type="checkbox" <?php if (isset($country_name_show) && $country_name_show) { ?> 
                                      checked="checked" <?php } ?>
                                  value="country" name="country_name_show" class="hide-column-tog"><?php echo INTERNALFLIGHT_CITY_TITLE; ?></label>
                ...
                <fieldset class="screen-options">
                    ...
                    <label for="internalflights_per_page"><?php echo NUMBER_OF_ITEMS_PER_PAGE_DESC; ?></label>
                    <input type="number" value="<?php
                    if (isset($internalflights_per_page)) {
                        echo $internalflights_per_page;
                    } else {
                        echo INTERNALFLIGHTS_PER_PAGE_DEFAULT;
                    }

                    ?>" maxlength="3" id="internalflights_per_page" name="internalflights_per_page" class="screen-per-page" max="999" min="1" step="1">
                </fieldset>
    ...
    <h1>
        <?php echo DASHBOARD_INTERNALFLIGHT_TITLE; ?> 
        <a class="page-title-action" onclick="getAddNewPage(this)" ><?php echo ADD_NEW_TITLE; ?></a>
    </h1>

    <?php $this->renderFeedbackMessages(); ?>

    <form id="form-internalflight-edit" method="post" onsubmit="submitFormInternalflightEdit(event)">
        ...
        <p class="search-box">
            <label for="internalflight-search-input" class="screen-reader-text">
                <?php echo SEARCH_INTERNALFLIGHTS_TITLE; ?>:</label>
            <input type="search" value="<?php
            if (isset($this->s)) {
                echo htmlspecialchars($this->s);
            }

            ?>" name="s" id="internalflight-search-input" />
            <input type="submit" value="<?php echo SEARCH_INTERNALFLIGHTS_TITLE; ?>" class="button" id="search-submit" />
        </p>
        ...
        <h2 class="screen-reader-text"><?php echo INTERNALFLIGHTS_LIST_TITLE; ?></h2>
        <table class="wp-list-table widefat fixed striped internalflights">
            <thead>
                <tr>
                    ...
                    if (isset($this->orderby) && $this->orderby == "post_title" && in_array($this->order, array('asc', 'desc'))) {
                        ...
                        <th class="manage-column column-post_title sorted <?php echo $this->order; ?>" id="post_title" scope="col">
                            <a href="#" orderby="post_title" order="<?php echo $this->order; ?>" onclick="filterOrderBy(this)">
                                <span><?php echo INTERNALFLIGHT_NAME_TITLE; ?></span>
                    ...
                    if (isset($this->orderby) && $this->orderby == "city_name" && in_array($this->order, array('asc', 'desc'))) {
                        ...
                        ?> sorted <?php echo $this->order; ?>" id="city_name" scope="col">
                            <a href="#" orderby="city_name" order="<?php echo $this->order; ?>" onclick="filterOrderBy(this)">
                                <span><?php echo INTERNALFLIGHT_CITY_TITLE; ?></span>
                                <span class="sorting-indicator"></span>
                            </a>
                        </th>   
                        <?php
            ...
            <tbody data-wp-lists="list:internalflight" id="the-list">
                <?php
                if (!is_null($this->internalflightList)) {
                    foreach ($this->internalflightList as $internalflightInfo) {

                        ?>
                        <tr id="internalflight-<?php echo $internalflightInfo->ID; ?>">
                            <th class="check-column" scope="row">
                                <label for="internalflight_<?php echo $internalflightInfo->ID; ?>" class="screen-reader-text"><?php echo SELECT_TITLE; ?> <?php echo htmlspecialchars($internalflightInfo->post_title); ?></label>
                                <input type="checkbox" value="<?php echo $internalflightInfo->ID; ?>" class="author" id="internalflight_<?php echo $internalflightInfo->ID; ?>" name="internalflights[]" >
                            </th>

                            <td data-colname="<?php echo INTERNALFLIGHT_NAME_TITLE; ?>" class="username column-username has-row-actions column-primary">
                                <img width="32" height="32" class="image image-32 photo" srcset="<?php
                                if (isset($internalflightInfo->image_url)) {
                                    echo URL . htmlspecialchars($internalflightInfo->image_url);
                                } else {
                                ...
                                ?>" src="<?php
                                     ...
                                     ?>" alt=""> 
                                <strong>
                                    <a href="#" internalflight="<?php echo $internalflightInfo->ID; ?>" name="<?php echo htmlspecialchars($internalflightInfo->post_name); ?>" onclick="getInternalflightInfoPage(this)"><?php echo htmlspecialchars($internalflightInfo->post_title); ?></a>
                                </strong>
                                <br>
                                <div class="row-actions">
                                    <span class="view">
                                        <a href="#" internalflight="<?php echo $internalflightInfo->ID; ?>" name="<?php echo htmlspecialchars($internalflightInfo->post_name); ?>" onclick="viewInternalflight(this)"><?php echo VIEW_TITLE; ?>
                                        </a>
                                    </span> |
                                    <span class="edit">
                                        <a href="#" internalflight="<?php echo $internalflightInfo->ID; ?>" name="<?php echo htmlspecialchars($internalflightInfo->post_name); ?>" onclick="getEditInternalflightPage(this)"><?php echo EDIT_TITLE; ?>
                                        </a>
                                    </span> |
                                    <span class="delete">
                                        <a href="#" class="submitdelete" internalflight="<?php echo $internalflightInfo->ID; ?>" name="<?php echo htmlspecialchars($internalflightInfo->post_title); ?>" onclick="deleteInternalflight(this)"><?php echo DELETE_TITLE; ?>
                                        </a>
                                    </span>
                                </div>
                            ...
                            <td data-colname="<?php echo INTERNALFLIGHT_CITY_TITLE; ?>" class="slug column-city <?php
                            if (!(isset($city_name_show) && $city_name_show)) {
                                echo " hidden ";
                            }

                            ?>"><?php echo htmlspecialchars($internalflightInfo->city_name); ?></td>
                            ...
                            ?>"><?php echo htmlspecialchars($internalflightInfo->country_name); ?></td>
                            ...
                            ?>"><?php echo htmlspecialchars($internalflightInfo->current_rating); ?></td>
                            ...
                            ?>"><?php echo htmlspecialchars($internalflightInfo->vote_times); ?></td>
                        </tr>      
                        <?php
                    }
                }
...
if (!(isset($this->ajax) && $this->ajax)) {
    ...
    <script>
        jQuery("#adv-settings").submit(function (e) {
            ...
            var postDataSearch = jQuery("#form-internalflight-edit").serializeArray();
            ...
            searchInternalflight(postData);
        });

        //        jQuery("#form-internalflight-edit").submit(function (e) {
        //            e.preventDefault(); //STOP default action
        //            var postData = jQuery(this).serializeArray();
        //            searchInternalflight(postData);
        //        });

        function submitFormInternalflightEdit(e) {
            e.preventDefault(); //STOP default action
            try {
                var postData = jQuery("#form-internalflight-edit").serializeArray();
                searchInternalflight(postData);
            } catch (e) {
        ...
        function searchInternalflight(postData) {
            jQuery.ajax({
                url: "",
                type: "POST",
                data: postData,
        ...
        function filterOrderBy(element) {
            ...
            jQuery('#form-internalflight-edit input[name="orderby"]').val(orderby);
            jQuery('#form-internalflight-edit input[name="order"]').val(order);
            var postData = jQuery("#form-internalflight-edit").serializeArray();
        ...
        function filterPage(element) {
            ...
            jQuery('#form-internalflight-edit input[name="page"]').val(page);
            var postData = jQuery("#form-internalflight-edit").serializeArray();
            //        var formURL = jQuery(this).attr("action");
            searchInternalflight(postData);
        }

        function getEditInternalflightPage(element) {
            var ID = jQuery(element).attr("internalflight");
            var name = jQuery(element).attr("name");
            var url = "<?php echo URL . CONTEXT_PATH_INTERNALFLIGHT_EDIT_INFO; ?>" + ID + "/" + name;
        ...
        function getAddNewPage(element) {
            var url = "<?php echo URL . CONTEXT_PATH_INTERNALFLIGHT_ADD_NEW; ?>";
            if (url != null && url != "" && url != undefined) {
                var win = window.open(url, '_blank');
                win.focus();
            }
        ...
        function getInternalflightInfoPage(element) {
            var internalflight = jQuery(element).attr("internalflight");
            var name = jQuery(element).attr("name");
            var url = "<?php echo URL . CONTEXT_PATH_INTERNALFLIGHT_INFO; ?>" + internalflight + "/" + name;
            if (url != null && url != "" && url != undefined) {
                var win = window.open(url, '_blank');
                win.focus();
            }
        }
        function viewInternalflight(element) {
            var internalflight = jQuery(element).attr("internalflight");
            var name = jQuery(element).attr("name");
            var url = "<?php echo URL . CONTEXT_PATH_INTERNALFLIGHT_VIEW; ?>" + internalflight + "/" + name;
            if (url != null && url != "" && url != undefined) {
                var win = window.open(url, '_blank');
                win.focus();
            }
        ...
        function deleteInternalflight(element) {
            var internalflight = jQuery(element).attr("internalflight");
            var name = jQuery(element).attr("name");
            if (confirm('<?php echo CONFIRM_DELETE_INTERNALFLIGHT; ?>' + name + '<?php echo CONFIRM_DELETE_CANCEL_OK; ?>')) {
                jQuery("#cb-select-all-1").prop('checked', false);
                jQuery("#cb-select-all-2").prop('checked', false);
                jQuery("input[name='internalflights[]'][type=checkbox]").prop('checked', false);
                jQuery("input[name='internalflights[]'][type=checkbox][value='" + internalflight + "']").prop('checked', true);

                jQuery('#form-internalflight-edit select[name="action"] option').removeAttr('selected');
                jQuery('#form-internalflight-edit select[name="action2"] option').removeAttr('selected');
                jQuery('#form-internalflight-edit select[name="action"] option[value="delete"]').attr('selected', true);
                jQuery('#form-internalflight-edit input[name="type"]').val('action');
                var postData = jQuery("#form-internalflight-edit").serializeArray();
                //        var formURL = jQuery(this).attr("action");
                searchInternalflight(postData);
        ...
        function applyAction(type) {
            if (confirm('<?php echo CONFIRM_ACTION_DESTINATION . CONFIRM_ACTION_CANCEL_OK; ?>')) {
                jQuery('#form-internalflight-edit input[name="type"]').val(type);
                var postData = jQuery("#form-internalflight-edit").serializeArray();
                //        var formURL = jQuery(this).attr("action");
                searchInternalflight(postData);
            }
        ...
        function checkAll(element) {
            if (jQuery(element).prop('checked')) {
                ...
                jQuery("input[name='internalflights[]'][type=checkbox]").prop('checked', true);
            } else {
                jQuery("#cb-select-all-1").prop('checked', false);
                jQuery("#cb-select-all-2").prop('checked', false);
                jQuery("input[name='internalflights[]'][type=checkbox]").prop('checked', false);
