...
if (isset($userBO->manage_activities_columns_show)) {
    if (isset($userBO->manage_activities_columns_show->city_name_show)) {
        $city_name_show = $userBO->manage_activities_columns_show->city_name_show;
    } else {
        $city_name_show = true;
    }
    if (isset($userBO->manage_activities_columns_show->country_name_show)) {
        $country_name_show = $userBO->manage_activities_columns_show->country_name_show;
    } else {
        $country_name_show = true;
    }
    if (isset($userBO->manage_activities_columns_show->current_rating_show)) {
        $current_rating_show = $userBO->manage_activities_columns_show->current_rating_show;
    } else {
        $current_rating_show = true;
    }
    if (isset($userBO->manage_activities_columns_show->vote_times_show)) {
        $vote_times_show = $userBO->manage_activities_columns_show->vote_times_show;
    } else {
...
if (isset($userBO->activities_per_page)) {
    $activities_per_page = $userBO->activities_per_page;
} else if (isset($_SESSION['options']) && isset($_SESSION['options']->activities_per_page)) {
    $activities_per_page = $_SESSION['options']->activities_per_page;
} else {
...
if (!(isset($this->ajax) && $this->ajax)) {
            ...
            <form method="post" id="adv-settings" >
                <fieldset class="metabox-prefs">
                    ...
                    <label><input type="checkbox" <?php if (isset($city_name_show) && $city_name_show) { ?> 
                                      checked="checked" <?php } ?>
                                  value="city" name="city_name_show" class="hide-column-tog"><?php echo ACTIVITY_CITY_TITLE; ?></label>
                                  <label><input type="checkbox" <?php if (isset($country_name_show) && $country_name_show) { ?> 
                                      checked="checked" <?php } ?>
                                  value="country" name="country_name_show" class="hide-column-tog"><?php echo ACTIVITY_CITY_TITLE; ?></label>
                ...
                <fieldset class="screen-options">
                    ...
                    <label for="activities_per_page"><?php echo NUMBER_OF_ITEMS_PER_PAGE_DESC; ?></label>
                    <input type="number" value="<?php
                    if (isset($activities_per_page)) {
                        echo $activities_per_page;
                    } else {
                        echo ACTIVITIES_PER_PAGE_DEFAULT;
                    }

                    ?>" maxlength="3" id="activities_per_page" name="activities_per_page" class="screen-per-page" max="999" min="1" step="1">
                </fieldset>
    ...
    <h1>
        <?php echo DASHBOARD_ACTIVITY_TITLE; ?> 
        <a class="page-title-action" onclick="getAddNewPage(this)" ><?php echo ADD_NEW_TITLE; ?></a>
    </h1>

    <?php $this->renderFeedbackMessages(); ?>

    <form id="form-activity-edit" method="post" onsubmit="submitFormActivityEdit(event)">
        ...
        <p class="search-box">
            <label for="activity-search-input" class="screen-reader-text">
                <?php echo SEARCH_ACTIVITIES_TITLE; ?>:</label>
            <input type="search" value="<?php
            if (isset($this->s)) {
                echo htmlspecialchars($this->s);
            }

            ?>" name="s" id="activity-search-input" />
            <input type="submit" value="<?php echo SEARCH_ACTIVITIES_TITLE; ?>" class="button" id="search-submit" />
        </p>
        ...
        <h2 class="screen-reader-text"><?php echo ACTIVITIES_LIST_TITLE; ?></h2>
        <table class="wp-list-table widefat fixed striped activities">
            <thead>
                <tr>
                    ...
                    if (isset($this->orderby) && $this->orderby == "post_title" && in_array($this->order, array('asc', 'desc'))) {
                        ...
                        <th class="manage-column column-post_title sorted <?php echo $this->order; ?>" id="post_title" scope="col">
                            <a href="#" orderby="post_title" order="<?php echo $this->order; ?>" onclick="filterOrderBy(this)">
                                <span><?php echo ACTIVITY_NAME_TITLE; ?></span>
                    ...
                    if (isset($this->orderby) && $this->orderby == "city_name" && in_array($this->order, array('asc', 'desc'))) {
                        ...
                        ?> sorted <?php echo $this->order; ?>" id="city_name" scope="col">
                            <a href="#" orderby="city_name" order="<?php echo $this->order; ?>" onclick="filterOrderBy(this)">
                                <span><?php echo ACTIVITY_CITY_TITLE; ?></span>
                                <span class="sorting-indicator"></span>
                            </a>
                        </th>   
                        <?php
            ...
            <tbody data-wp-lists="list:activity" id="the-list">
                <?php
                if (!is_null($this->activityList)) {
                    foreach ($this->activityList as $activityInfo) {

                        ?>
                        <tr id="activity-<?php echo $activityInfo->ID; ?>">
                            <th class="check-column" scope="row">
                                <label for="activity_<?php echo $activityInfo->ID; ?>" class="screen-reader-text"><?php echo SELECT_TITLE; ?> <?php echo htmlspecialchars($activityInfo->post_title); ?></label>
                                <input type="checkbox" value="<?php echo $activityInfo->ID; ?>" class="author" id="activity_<?php echo $activityInfo->ID; ?>" name="activities[]" >
                            </th>

                            <td data-colname="<?php echo ACTIVITY_NAME_TITLE; ?>" class="username column-username has-row-actions column-primary">
                                <img width="32" height="32" class="image image-32 photo" srcset="<?php
                                if (isset($activityInfo->image_url)) {
                                    echo URL . htmlspecialchars($activityInfo->image_url);
                                } else {
                                ...
                                ?>" src="<?php
                                     ...
                                     ?>" alt=""> 
                                <strong>
                                    <a href="#" activity="<?php echo $activityInfo->ID; ?>" name="<?php echo htmlspecialchars($activityInfo->post_name); ?>" onclick="getActivityInfoPage(this)"><?php echo htmlspecialchars($activityInfo->post_title); ?></a>
                                </strong>
                                <br>
                                <div class="row-actions">
                                    <span class="view">
                                        <a href="#" activity="<?php echo $activityInfo->ID; ?>" name="<?php echo htmlspecialchars($activityInfo->post_name); ?>" onclick="viewActivity(this)"><?php echo VIEW_TITLE; ?>
                                        </a>
                                    </span> |
                                    <span class="edit">
                                        <a href="#" activity="<?php echo $activityInfo->ID; ?>" name="<?php echo htmlspecialchars($activityInfo->post_name); ?>" onclick="getEditActivityPage(this)"><?php echo EDIT_TITLE; ?>
                                        </a>
                                    </span> |
                                    <span class="delete">
                                        <a href="#" class="submitdelete" activity="<?php echo $activityInfo->ID; ?>" name="<?php echo htmlspecialchars($activityInfo->post_title); ?>" onclick="deleteActivity(this)"><?php echo DELETE_TITLE; ?>
                                        </a>
                                    </span>
                                </div>
                            ...
                            <td data-colname="<?php echo ACTIVITY_CITY_TITLE; ?>" class="slug column-city <?php
                            if (!(isset($city_name_show) && $city_name_show)) {
                                echo " hidden ";
                            }

                            ?>"><?php echo htmlspecialchars($activityInfo->city_name); ?></td>
                            ...
                            ?>"><?php echo htmlspecialchars($activityInfo->country_name); ?></td>
                            ...
                            ?>"><?php echo htmlspecialchars($activityInfo->current_rating); ?></td>
                            ...
                            ?>"><?php echo htmlspecialchars($activityInfo->vote_times); ?></td>
                        </tr>      
                        <?php
                    }
                }
...
if (!(isset($this->ajax) && $this->ajax)) {
    ...
    <script>
        jQuery("#adv-settings").submit(function (e) {
            ...
            var postDataSearch = jQuery("#form-activity-edit").serializeArray();
            ...
            searchActivity(postData);
        });

        //        jQuery("#form-activity-edit").submit(function (e) {
        //            e.preventDefault(); //STOP default action
        //            var postData = jQuery(this).serializeArray();
        //            searchActivity(postData);
        //        });

        function submitFormActivityEdit(e) {
            e.preventDefault(); //STOP default action
            try {
                var postData = jQuery("#form-activity-edit").serializeArray();
                searchActivity(postData);
            } catch (e) {
        ...
        function searchActivity(postData) {
            jQuery.ajax({
                url: "",
                type: "POST",
                data: postData,
        ...
        function filterOrderBy(element) {
            ...
            jQuery('#form-activity-edit input[name="orderby"]').val(orderby);
            jQuery('#form-activity-edit input[name="order"]').val(order);
            var postData = jQuery("#form-activity-edit").serializeArray();
        ...
        function filterPage(element) {
            ...
            jQuery('#form-activity-edit input[name="page"]').val(page);
            var postData = jQuery("#form-activity-edit").serializeArray();
            //        var formURL = jQuery(this).attr("action");
            searchActivity(postData);
        }
        ...
        function getEditActivityPage(element) {
            var ID = jQuery(element).attr("activity");
            var name = jQuery(element).attr("name");
            var url = "<?php echo URL . CONTEXT_PATH_ACTIVITY_EDIT_INFO; ?>" + ID + "/" + name;
        ...
        function getAddNewPage(element) {
            var url = "<?php echo URL . CONTEXT_PATH_ACTIVITY_ADD_NEW; ?>";
            if (url != null && url != "" && url != undefined) {
                var win = window.open(url, '_blank');
                win.focus();
            }
        ...
        function getActivityInfoPage(element) {
            var activity = jQuery(element).attr("activity");
            var name = jQuery(element).attr("name");
            var url = "<?php echo URL . CONTEXT_PATH_ACTIVITY_INFO; ?>" + activity + "/" + name;
            if (url != null && url != "" && url != undefined) {
                var win = window.open(url, '_blank');
                win.focus();
            }
        ...
        function viewActivity(element) {
            var activity = jQuery(element).attr("activity");
            var name = jQuery(element).attr("name");
            var url = "<?php echo URL . CONTEXT_PATH_ACTIVITY_VIEW; ?>" + activity + "/" + name;
            if (url != null && url != "" && url != undefined) {
                var win = window.open(url, '_blank');
                win.focus();
            }
        ...
        function deleteActivity(element) {
            var activity = jQuery(element).attr("activity");
            var name = jQuery(element).attr("name");
            if (confirm('<?php echo CONFIRM_DELETE_ACTIVITY; ?>' + name + '<?php echo CONFIRM_DELETE_CANCEL_OK; ?>')) {
                jQuery("#cb-select-all-1").prop('checked', false);
                jQuery("#cb-select-all-2").prop('checked', false);
                jQuery("input[name='activities[]'][type=checkbox]").prop('checked', false);
                jQuery("input[name='activities[]'][type=checkbox][value='" + activity + "']").prop('checked', true);

                jQuery('#form-activity-edit select[name="action"] option').removeAttr('selected');
                jQuery('#form-activity-edit select[name="action2"] option').removeAttr('selected');
                jQuery('#form-activity-edit select[name="action"] option[value="delete"]').attr('selected', true);
                jQuery('#form-activity-edit input[name="type"]').val('action');
                var postData = jQuery("#form-activity-edit").serializeArray();
                //        var formURL = jQuery(this).attr("action");
                searchActivity(postData);
        ...
        function applyAction(type) {
            if (confirm('<?php echo CONFIRM_ACTION_DESTINATION . CONFIRM_ACTION_CANCEL_OK; ?>')) {
                jQuery('#form-activity-edit input[name="type"]').val(type);
                var postData = jQuery("#form-activity-edit").serializeArray();
                //        var formURL = jQuery(this).attr("action");
                searchActivity(postData);
            }
        ...
        function checkAll(element) {
            if (jQuery(element).prop('checked')) {
                ...
                jQuery("input[name='activities[]'][type=checkbox]").prop('checked', true);
            } else {
                jQuery("#cb-select-all-1").prop('checked', false);
                jQuery("#cb-select-all-2").prop('checked', false);
                jQuery("input[name='activities[]'][type=checkbox]").prop('checked', false);
