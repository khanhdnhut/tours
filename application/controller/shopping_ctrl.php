...
class ShoppingCtrl extends Controller
{
    ...
    public function addNew()
    {
        if (in_array(Auth::getCapability(), array(CAPABILITY_ADMINISTRATOR))) {
            Model::autoloadModel('shopping');
            $model = new ShoppingModel($this->db);
...
//            $taxonomyModel->getAllSorted($this->view->cityList, $taxonomyModel->buildTree($taxonomyModel->getAll("city")), -1);
            ...
            if (isset($_POST['ajax']) && !is_null($_POST['ajax'])) {
                $this->view->renderAdmin(RENDER_VIEW_SHOPPING_ADD_NEW, TRUE);
            } else {
                $this->view->renderAdmin(RENDER_VIEW_SHOPPING_ADD_NEW);
            }
        } else {
            $this->login();
        }
    ...
    public function editInfo($post_id = NULL)
    {
        if (in_array(Auth::getCapability(), array(CAPABILITY_ADMINISTRATOR))) {
            ...
            if (isset($_POST['shopping'])) {
                $this->para->post_id = $_POST['shopping'];
            ...
            if (isset($this->para->post_id)) {
                ...
                $this->view->shoppingBO = $model->get($this->para->post_id);
                ...
                $this->view->cityList = new SplDoublyLinkedList();
                if (isset($this->view->shoppingBO->country_id) && $this->view->shoppingBO->country_id != "0") {
                    $taxonomyModel->getAllSorted($this->view->cityList, $taxonomyModel->buildTree($taxonomyModel->getByMetaData("city", "country", $this->view->shoppingBO->country_id)), -1);
                }
                if (isset($this->view->para) && isset($this->view->para->country_id)) {
                    $taxonomyModel->getAllSorted($this->view->cityList, $taxonomyModel->buildTree($taxonomyModel->getByMetaData("city", "country", $this->view->para->country_id)), -1);
                }
                ...
                if (isset($post_id) && !is_null($post_id)) {
                    $this->view->renderAdmin(RENDER_VIEW_SHOPPING_EDIT);
                } else {
                    $this->view->renderAdmin(RENDER_VIEW_SHOPPING_EDIT, TRUE);
                }
            } else {
                header('location: ' . URL . CONTEXT_PATH_SHOPPING_INDEX);
            }
        } else {
            $this->login();
        }
    ...
    public function info($post_id = NULL)
    {
        if (in_array(Auth::getCapability(), array(CAPABILITY_ADMINISTRATOR))) {
            ...
            if (isset($this->para->post_id)) {
                ...
                if (isset($post_id) && !is_null($post_id)) {
                    $this->view->renderAdmin(RENDER_VIEW_SHOPPING_INFO);
                } else {
                    $this->view->renderAdmin(RENDER_VIEW_SHOPPING_INFO, TRUE);
                }
            } else {
                header('location: ' . URL . CONTEXT_PATH_SHOPPING_INDEX);
            }
    ...
    public function view($post_id = NULL)
    {
        ...
        if (isset($this->para->post_id)) {
            ...
            if (isset($post_id) && !is_null($post_id)) {
                $this->view->render(RENDER_VIEW_SHOPPING);
            } else {
                $this->view->render(RENDER_VIEW_SHOPPING, TRUE);
            }
        } else {
            header('location: ' . URL);
        }
    ...
    public function index()
    {
        if (in_array(Auth::getCapability(), array(CAPABILITY_ADMINISTRATOR))) {
            ...
            if (isset($_POST['shoppings'])) {
                $this->para->shoppings = $_POST['shoppings'];
            }
            ...
            if (isset($_POST['shoppings_per_page'])) {
                $this->para->shoppings_per_page = $_POST['shoppings_per_page'];
            }
            ...
            if (isset($this->para->type) && in_array($this->para->type, array("action", "action2")) && isset($this->para->shoppings)) {
            ...
            if (count((array) $this->para) > 0) {
                $this->view->ajax = TRUE;
                $this->view->renderAdmin(RENDER_VIEW_SHOPPING_INDEX, TRUE);
            } else {
                $this->view->renderAdmin(RENDER_VIEW_SHOPPING_INDEX);
            }
        } else {
            $this->login();
        }
