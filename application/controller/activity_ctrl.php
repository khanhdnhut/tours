...
class ActivityCtrl extends Controller
{
    ...
    public function addNew()
    {
        if (in_array(Auth::getCapability(), array(CAPABILITY_ADMINISTRATOR))) {
            Model::autoloadModel('activity');
            $model = new ActivityModel($this->db);
...
//            $taxonomyModel->getAllSorted($this->view->cityList, $taxonomyModel->buildTree($taxonomyModel->getAll("city")), -1);
            ...
            if (isset($_POST['ajax']) && !is_null($_POST['ajax'])) {
                $this->view->renderAdmin(RENDER_VIEW_ACTIVITY_ADD_NEW, TRUE);
            } else {
                $this->view->renderAdmin(RENDER_VIEW_ACTIVITY_ADD_NEW);
            }
        } else {
            $this->login();
        }
    ...
    public function editInfo($post_id = NULL)
    {
        if (in_array(Auth::getCapability(), array(CAPABILITY_ADMINISTRATOR))) {
            ...
            if (isset($_POST['activity'])) {
                $this->para->post_id = $_POST['activity'];
            ...
            if (isset($this->para->post_id)) {
                ...
                $this->view->activityBO = $model->get($this->para->post_id);
                ...
                $this->view->cityList = new SplDoublyLinkedList();
                if (isset($this->view->activityBO->country_id) && $this->view->activityBO->country_id != "0") {
                    $taxonomyModel->getAllSorted($this->view->cityList, $taxonomyModel->buildTree($taxonomyModel->getByMetaData("city", "country", $this->view->activityBO->country_id)), -1);
                }
                if (isset($this->view->para) && isset($this->view->para->country_id)) {
                    $taxonomyModel->getAllSorted($this->view->cityList, $taxonomyModel->buildTree($taxonomyModel->getByMetaData("city", "country", $this->view->para->country_id)), -1);
                }
                ...
                if (isset($post_id) && !is_null($post_id)) {
                    $this->view->renderAdmin(RENDER_VIEW_ACTIVITY_EDIT);
                } else {
                    $this->view->renderAdmin(RENDER_VIEW_ACTIVITY_EDIT, TRUE);
                }
            } else {
                header('location: ' . URL . CONTEXT_PATH_ACTIVITY_INDEX);
            }
        } else {
            $this->login();
        }
    ...
    public function info($post_id = NULL)
    {
        if (in_array(Auth::getCapability(), array(CAPABILITY_ADMINISTRATOR))) {
            ...
            if (isset($this->para->post_id)) {
                ...
                if (isset($post_id) && !is_null($post_id)) {
                    $this->view->renderAdmin(RENDER_VIEW_ACTIVITY_INFO);
                } else {
                    $this->view->renderAdmin(RENDER_VIEW_ACTIVITY_INFO, TRUE);
                }
            } else {
                header('location: ' . URL . CONTEXT_PATH_ACTIVITY_INDEX);
            }
    ...
    public function view($post_id = NULL)
    {
        ...
        if (isset($this->para->post_id)) {
            ...
            if (isset($post_id) && !is_null($post_id)) {
                $this->view->render(RENDER_VIEW_ACTIVITY);
            } else {
                $this->view->render(RENDER_VIEW_ACTIVITY, TRUE);
            }
        } else {
            header('location: ' . URL);
        }
    ...
    public function index()
    {
        if (in_array(Auth::getCapability(), array(CAPABILITY_ADMINISTRATOR))) {
            ...
            if (isset($_POST['activities'])) {
                $this->para->activities = $_POST['activities'];
            }
            ...
            if (isset($_POST['activities_per_page'])) {
                $this->para->activities_per_page = $_POST['activities_per_page'];
            }
            ...
            if (isset($this->para->type) && in_array($this->para->type, array("action", "action2")) && isset($this->para->activities)) {
            ...
            if (count((array) $this->para) > 0) {
                $this->view->ajax = TRUE;
                $this->view->renderAdmin(RENDER_VIEW_ACTIVITY_INDEX, TRUE);
            } else {
                $this->view->renderAdmin(RENDER_VIEW_ACTIVITY_INDEX);
            }
        } else {
            $this->login();
        }
