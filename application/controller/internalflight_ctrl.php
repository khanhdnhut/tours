 ...
 *
 */
class InternalflightCtrl extends Controller
{
...
//            $taxonomyModel->getAllSorted($this->view->cityList, $taxonomyModel->buildTree($taxonomyModel->getAll("city")), -1);
            ...
            if (isset($_POST['ajax']) && !is_null($_POST['ajax'])) {
                $this->view->renderAdmin(RENDER_VIEW_INTERNALFLIGHT_ADD_NEW, TRUE);
            } else {
                $this->view->renderAdmin(RENDER_VIEW_INTERNALFLIGHT_ADD_NEW);
            }
        } else {
            $this->login();
        }
        ...
        if (in_array(Auth::getCapability(), array(CAPABILITY_ADMINISTRATOR))) {
            ...
            if (isset($this->para->post_id)) {
                ...
                if (isset($post_id) && !is_null($post_id)) {
                    $this->view->renderAdmin(RENDER_VIEW_INTERNALFLIGHT_EDIT);
                } else {
                    $this->view->renderAdmin(RENDER_VIEW_INTERNALFLIGHT_EDIT, TRUE);
                }
            } else {
                header('location: ' . URL . CONTEXT_PATH_INTERNALFLIGHT_INDEX);
            }
        } else {
            $this->login();
        }
    ...
    public function info($post_id = NULL)
    {
        if (in_array(Auth::getCapability(), array(CAPABILITY_ADMINISTRATOR))) {
            ...
            if (isset($this->para->post_id)) {
                ...
                if (isset($post_id) && !is_null($post_id)) {
                    $this->view->renderAdmin(RENDER_VIEW_INTERNALFLIGHT_INFO);
                } else {
                    $this->view->renderAdmin(RENDER_VIEW_INTERNALFLIGHT_INFO, TRUE);
                }
            } else {
                header('location: ' . URL . CONTEXT_PATH_INTERNALFLIGHT_INDEX);
            }
