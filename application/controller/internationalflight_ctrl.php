 ...
 *
 */
class InternationalflightCtrl extends Controller
{
    ...
    public function addNew()
    {
        if (in_array(Auth::getCapability(), array(CAPABILITY_ADMINISTRATOR))) {
            Model::autoloadModel('internationalflight');
            $model = new InternationalflightModel($this->db);
...
//            $taxonomyModel->getAllSorted($this->view->cityList, $taxonomyModel->buildTree($taxonomyModel->getAll("city")), -1);
            ...
            if (isset($_POST['ajax']) && !is_null($_POST['ajax'])) {
                $this->view->renderAdmin(RENDER_VIEW_INTERNATIONALFLIGHT_ADD_NEW, TRUE);
            } else {
                $this->view->renderAdmin(RENDER_VIEW_INTERNATIONALFLIGHT_ADD_NEW);
            }
        } else {
            $this->login();
        }
    ...
    public function editInfo($post_id = NULL)
    {
        if (in_array(Auth::getCapability(), array(CAPABILITY_ADMINISTRATOR))) {
            ...
            $model = new InternationalflightModel($this->db);
            $this->para = new stdClass();
            if (isset($_POST['internationalflight'])) {
                $this->para->post_id = $_POST['internationalflight'];
            ...
            if (isset($this->para->post_id)) {
                ...
                $this->view->internationalflightBO = $model->get($this->para->post_id);
                ...
                $this->view->cityList = new SplDoublyLinkedList();
                if (isset($this->view->internationalflightBO->country_id) && $this->view->internationalflightBO->country_id != "0") {
                    $taxonomyModel->getAllSorted($this->view->cityList, $taxonomyModel->buildTree($taxonomyModel->getByMetaData("city", "country", $this->view->internationalflightBO->country_id)), -1);
                }
                if (isset($this->view->para) && isset($this->view->para->country_id)) {
                    $taxonomyModel->getAllSorted($this->view->cityList, $taxonomyModel->buildTree($taxonomyModel->getByMetaData("city", "country", $this->view->para->country_id)), -1);
                }
                ...
                if (isset($post_id) && !is_null($post_id)) {
                    $this->view->renderAdmin(RENDER_VIEW_INTERNATIONALFLIGHT_EDIT);
                } else {
                    $this->view->renderAdmin(RENDER_VIEW_INTERNATIONALFLIGHT_EDIT, TRUE);
                }
            } else {
                header('location: ' . URL . CONTEXT_PATH_INTERNATIONALFLIGHT_INDEX);
            }
        } else {
            $this->login();
        }
    ...
    public function info($post_id = NULL)
    {
        if (in_array(Auth::getCapability(), array(CAPABILITY_ADMINISTRATOR))) {
            ...
            if (isset($this->para->post_id)) {
                ...
                if (isset($post_id) && !is_null($post_id)) {
                    $this->view->renderAdmin(RENDER_VIEW_INTERNATIONALFLIGHT_INFO);
                } else {
                    $this->view->renderAdmin(RENDER_VIEW_INTERNATIONALFLIGHT_INFO, TRUE);
                }
            } else {
                header('location: ' . URL . CONTEXT_PATH_INTERNATIONALFLIGHT_INDEX);
            }
    ...
    public function view($post_id = NULL)
    {
        ...
        if (isset($this->para->post_id)) {
            ...
            if (isset($post_id) && !is_null($post_id)) {
                $this->view->render(RENDER_VIEW_INTERNATIONALFLIGHT);
            } else {
                $this->view->render(RENDER_VIEW_INTERNATIONALFLIGHT, TRUE);
            }
        } else {
            header('location: ' . URL);
        }
    ...
    public function index()
    {
        if (in_array(Auth::getCapability(), array(CAPABILITY_ADMINISTRATOR))) {
            ...
            if (isset($_POST['internationalflights'])) {
                $this->para->internationalflights = $_POST['internationalflights'];
            }
            ...
            if (isset($_POST['internationalflights_per_page'])) {
                $this->para->internationalflights_per_page = $_POST['internationalflights_per_page'];
            }
            ...
            if (isset($this->para->type) && in_array($this->para->type, array("action", "action2")) && isset($this->para->internationalflights)) {
            ...
            if (count((array) $this->para) > 0) {
                $this->view->ajax = TRUE;
                $this->view->renderAdmin(RENDER_VIEW_INTERNATIONALFLIGHT_INDEX, TRUE);
            } else {
                $this->view->renderAdmin(RENDER_VIEW_INTERNATIONALFLIGHT_INDEX);
            }
        } else {
            $this->login();
        }
